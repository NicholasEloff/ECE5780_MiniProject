...
This was accomplished by interfacing our STM32 board to the transmitter from the remote. We first attempted to interface with the motors of the car but we were not able to provide enough current to run them. We then tried to use the circuit board of the car itself and this proved to be futile. After discussions and ideas from our TA we were able to interface with the board of the transmitter. We accomplished this by soldering wires onto each of the leads (left,right,forward,backward). We also soldered a wire that acted as ground.
...
To each of these four "staples", solder a jumper wire to them and connect them to a breadboard. for each of those, connect a 10k resistor in series to one of the 5v tolerant pins (in our code, we used PB6, 7, 8, 9). You will also need to connect pins with eachother and use 10k resistors for i2c communication for the gyro. 
